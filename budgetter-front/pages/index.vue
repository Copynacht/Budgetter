<template>
  <v-container>
    ...
    <v-row>
      ...
      <v-col cols="6">
        <v-btn cols="6" v-if="slide === 0" size="x-large" color="success" @click="loan()">
          貸し借り記録
        </v-btn>
        <v-btn cols="3" v-if="slide !== 0 && slide !== 5" size="x-large" color="success" @click="back()">
          戻る
        </v-btn>
        <v-btn cols="3" v-if="slide === 4 || slide === 5" size="x-large" class="ma-1" color="success" @click="reset()">
          リセット
        </v-btn>
      </v-col>
    </v-row>
      ...
      <v-carousel-item>
        <v-row no-gutters>
          <v-col v-if="selectedCategory" v-for="categoryDetail in activeCategoryDetails" :key="categoryDetail.id"
            cols="12" md="6" lg="4">
            <v-card class="ma-2 pa-2 custom-card" outlined @click="moveRegister(categoryDetail.id)">
              <v-card-title>
...
const moveCategoryDetailSelect = (id) => {
  cSelected.value = id
  categoryDetails.value = categories.value.find(category => category.id === id).details
...
const back = () => {
  if (slide.value === 2 && paymentDetails.value.length === 0) {
    slide.value -= 2
  } else if (slide.value === 4 && categoryDetails.value.length === 0) {
    slide.value -= 2
  } else {
    slide.value -= 1
    if (slide.value === 0) {
