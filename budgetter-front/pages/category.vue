        ...
        <draggable v-model="categories" :group="{ name: 'categories', pull: true, put: false }" item-key="id"
          ...
          <template #item="{ element: category }">
            <v-card class="mb-4 custom-p-card">
              ...
              <v-card-text>
                <!-- CategoryDetailドラッグ -->
                <draggable v-model="category.category_details" :group="{ name: 'category_details', pull: true, put: true }"
                  item-key="id" @end="onCategoryDetailOrderChangeAll" :animation="100" :fallbackOnBody="true"
...
async function submitNewCategory() {
  try {
    ...
    await $api.post('/api/categories/', {
      ...
      icon: iconValue,
      order: Math.max(...categories.value.map(p => p.order)) + 1,
    })
